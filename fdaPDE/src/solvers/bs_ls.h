//
// Created by Marco Galliani on 26/09/25.
//
#ifndef BS_LS_H
#define BS_LS_H
...
struct bs_ls {
   ...
   public:
    ...
    bs_ls(const std::string& formula, const GeoFrame& gf, InfoT&& info, const WeightMatrix& W) : W_(W) {
    ...
    template <typename GeoFrame, typename InfoT>
        requires(is_valid_info_t<InfoT>::value)
    bs_ls(const std::string& formula, const GeoFrame& gf, InfoT&& info) :
        bs_ls(formula, gf, info, vector_t::Ones(gf[0].rows()).asDiagonal()) { }
    // construct with no data
    template <typename GeoFrame, typename InfoT, typename WeightMatrix>
        requires(is_valid_info_t<InfoT>::value)
    bs_ls(const GeoFrame& gf, InfoT&& info, const WeightMatrix& W) : W_(W) {
        fdapde_static_assert(GeoFrame::Order == 1, THIS_CLASS_IS_FOR_ORDER_ONE_GEOFRAMES_ONLY);
        fdapde_assert(gf.n_layers() == 1);
        n_obs_  = gf[0].rows();
	    n_locs_ = n_obs_;
    ...
    template <typename GeoFrame, typename InfoT>
        requires(is_valid_info_t<InfoT>::value)
    bs_ls(const GeoFrame& gf, InfoT&& info) :
        bs_ls(gf, info, vector_t::Ones(gf[0].rows()).asDiagonal()) { }

    // perform splines based numerical discretization
    template <typename Penalty> void discretize(Penalty&& penalty) {
        ...
        TrialFunction u(bilinear_form.trial_space());
        TestFunction  v(bilinear_form.trial_space());

        R0_ = integral(bilinear_form.trial_space().triangulation())(u * v).assemble();
	    ...
	    b_.resize(2*n_dofs_, 1); // TODO: check if we need 2*n_dofs_
        return;
    }
    // non-parametric fit
    ...
    void analyze_data(const DataLocs& locs, const matrix_t& y, const matrix_t& X, const WeightMatrix& W) {
        ...
        n_covs_ = X.cols();
        eval_basis_at_(locs);   // update \Psi matrix
        if (require_woodbury_realloc) { U_ = matrix_t::Zero(2 * n_dofs_, n_covs_); } // TODO: check if we need 2*n_dofs_
        if (require_woodbury_realloc) { V_ = matrix_t::Zero(n_covs_, 2 * n_dofs_); } // TODO: check if we need 2*n_dofs_
...
template <typename BilinearForm, typename LinearForm> struct bs_ls {
    using solver_t = internals::bs_ls;
   private:
    struct info_t {
        std::tuple<BilinearForm, LinearForm> penalty;
    };
   public:
    bs_ls(const BilinearForm& bilinear_form, const LinearForm& linear_form) :
